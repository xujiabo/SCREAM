...
from datasets.kitti import KITTI_Train, KITTI_Val, KITTI_Test
...
def visualize(loader, dis_thresh, icp_thresh):
    processed = 0
    ...
    with torch.no_grad():
        for src_pcd, tgt_pcd, rot, trans, s, c in loader:
            s = s[0].item()
            src_pcd, tgt_pcd, rot, trans, c = src_pcd.to(device), tgt_pcd.to(device), rot.to(device), trans.to(
                device), c.to(device)
            with autocast():
            ...
            print(
                "\r进度：%s  point trans loss: %.5f  re: %.5f  te: %.5f  success rre: %.5f  success rte: %.5f  success rate: %.5f" % (
                    processbar(processed, len(loader)), point_loss.item(), re, te,
            ...
            ###############################  visualize #################################
            src_pred_pc = to_o3d_pcd(src_pred[0] / s + c, [3/255, 168/255, 158/255])
            ...
            # correspondence
            src_selected_points = o3d.PointCloud()
            src_selected_points.points = o3d.Vector3dVector(np.asarray(src_pc.points)[valid_ind.cpu().numpy()])
            src_selected_points.colors = o3d.Vector3dVector(np.array([[1, 0.706, 0]] * torch.sum(valid_ind, dim=0).item()))
            src_selected_points.normals = o3d.Vector3dVector(np.asarray(src_pc.normals)[valid_ind.cpu().numpy()])

            tgt_selected_points = o3d.PointCloud()
            tgt_selected_points.points = o3d.Vector3dVector(np.asarray(tgt_pc.points)[tgt_ind.cpu().numpy()])
            tgt_selected_points.colors = o3d.Vector3dVector(np.array([[3/255, 168/255, 158/255]] * tgt_ind.shape[0]))
            tgt_selected_points.normals = o3d.Vector3dVector(np.asarray(tgt_pc.normals)[tgt_ind.cpu().numpy()])

            # draw
            o3d.draw_geometries([src_pc, tgt_pc], width=1000, height=800, window_name="src tgt")
            o3d.draw_geometries([src_pred_pc, tgt_pc], width=1000, height=800, window_name="tgt pred")
            o3d.draw_geometries([src_selected_points, tgt_selected_points], width=1000, height=800, window_name="src tgt corr")
            o3d.draw_geometries([deepcopy(src_pc).transform(transform), tgt_pc], width=1000, height=800, window_name="registration")
            o3d.draw_geometries([deepcopy(src_pc).transform(T.cpu().numpy()), tgt_pc], width=1000, height=800, window_name="GT")

    point_trans_loss = point_trans_loss / len(loader)
    ...
    success_rate = success_rate / len(loader)
...
def visualize_train():
    visualize(
        DataLoader(KITTI_Train(), batch_size=1, shuffle=True),
...
def visualize_val():
    visualize(
        DataLoader(KITTI_Val(), batch_size=1, shuffle=False),
        dis_thresh=1.5, icp_thresh=1
    )
...
def visualize_test():
    visualize(
        DataLoader(KITTI_Test(), batch_size=1, shuffle=True),
        dis_thresh=1.5, icp_thresh=1
    )
